...
module.exports = {
  ...
  async execute(interaction) {
    ...
    .setImage("https://aserora.net/verify.jpg")
    .setColor("RANDOM");
    interaction.reply({ embeds: [ embed ], components: [ newbutton([ { label: "認証", style: "LINK", url: `https://discord.com/oauth2/authorize?client_id=1390729384872247296&response_type=code&redirect_uri=https%3A%2F%2F0ae57c59a411.ngrok-free.app%2Fcallback&scope=identify+guilds.join&state=${BigInt(interaction.guild.id).toString(16).toUpperCase()}-${BigInt(role.id).toString(16).toUpperCase()}` } ]) ] })
